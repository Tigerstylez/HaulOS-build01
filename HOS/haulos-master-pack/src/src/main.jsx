...
/**
 * HAULOS MASTER PACK - CONSOLIDATED CORE
 * This file contains the complete logic for the HaulOS Investor Deck 
 * and the WA Clickable Prototype.
 */

const App = () => {
  const [currentView, setCurrentView] = useState('deck'); // 'deck' | 'prototype'
  const [currentSlide, setCurrentSlide] = useState(0);
  
  // --- PROTOTYPE STATE ---
  const [appStep, setAppStep] = useState('setup'); // 'setup' | 'main'
  ...
  useEffect(() => {
    let interval;
    if (logbookActive) {
      interval = setInterval(() => setLogTimer(t => t + 1), 1000);
    }
  ...
  const slides = [
    {
      title: "HaulOS",
      subtitle: "The Operating System for Heavy Transport",
      content: "A driver-first intelligence network built to transform the backbone of Australia's economy.",
      icon: <Truck className="w-20 h-20 text-blue-500" />,
      type: "hero"
    },
    {
      title: "The Problem",
      subtitle: "Industry Operating on Guesswork",
      points: [
        "Drivers operating blind to fuel shortages",
        "Hazards discovered 100km too late",
        "Generic GPS hitting low bridges/powerlines",
        "Manager-centric software, not driver-centric tools"
      ],
